...
//! [`ASSETS`]`items/`. The shell fetches them from here as itself, with no
...
/// The `items` panel's module: its entry document.
pub const ITEMS_ENTRY: &str = "/ui/items/index.html";

/// Where the module's files are served from, beneath its base.
pub const ITEMS_DIR: &str = "/ui/items/";
...
mod tests {
    ...
    fn trunks_hashed_output_is_immutable_and_the_names_that_never_change_are_not() {
        assert!(is_hashed(
            "hlin-sample-checklist-module-ad726f0dc22e8941.js"
        ));
        assert!(is_hashed(
            "hlin-sample-checklist-module-ad726f0dc22e8941_bg.wasm"
        ));
        ...
        assert!(!is_hashed("hlin-sample-checklist-module.js"));
    }
    ...
    fn a_build_is_read_from_its_directory() {
        let dir = std::env::temp_dir().join(format!("checklist-module-{}", std::process::id()));
