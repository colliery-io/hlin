//! The converter's components: a number, two units, and the answer as you
//! type.
//!
//! Mounted twice, unchanged: at the root of the converter's own origin by
//! `hlin-widget-converter-ui`, and in a Hlin panel by
//! `hlin-widget-converter-module`. The whole widget is here. It asks its
//! platform for nothing, with either client (the platform's tests hold it to
//! that), so it works the same on a read-only surface, and the only thing it
//! keeps is what the person typed: handed to the shell on `suspend` when the
//! panel is scrolled away, and taken back from `restored` when it returns. Its
//! own page is never suspended, so there it simply stays.

mod units;

use hlin_widget_ui::use_widget;
use leptos::prelude::*;
use units::{QUANTITIES, Quantity, convert, quantity, written};

/// The converter's own look, against the `--hlin-*` tokens and the shared
...
pub fn Converter() -> impl IntoView {
    ...
    widget.on_suspend(move || Some(typed.with_untracked(Typed::kept)));

    let answer =
        move || typed.with(|typed| convert(typed.quantity, &typed.value, &typed.from, &typed.to));
