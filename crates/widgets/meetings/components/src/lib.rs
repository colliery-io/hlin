//! The meetings widget's components: today's meetings in your own time zone,
//! which one is on now, and your answer to each.
//!
//! Mounted twice, unchanged: at the root of the widget's own origin by
//! `hlin-widget-meetings-ui`, and in a Hlin panel by
//! `hlin-widget-meetings-module`. The widget sends instants; the browser
//! knows where it is, so times are shown in its zone, and "now" and "next"
//! follow its clock, checked every half minute.
...
/// The meetings widget's own look, against the `--hlin-*` tokens and the
/// shared classes in `hlin-widget-ui`'s stylesheet.
...
pub fn Meetings() -> impl IntoView {
    ...
    let drawn = loaded_view(meetings, move |meetings| {
