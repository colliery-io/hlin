//! The shoutbox's components: the conversation, newest at the bottom, and a
//! line to say something on.
//!
//! Mounted twice, unchanged: at the root of the shoutbox's own origin by
//! `hlin-widget-shoutbox-ui`, and in a Hlin panel by
//! `hlin-widget-shoutbox-module`. Everything shared with the other widgets
//! (fetching again when told, writes and their refusals) is
//! `hlin-widget-ui`'s; what is here is what the shoutbox draws and asks for.

use hlin_widget_ui::{Request, loaded_view, use_widget};
use leptos::prelude::*;
use serde::Deserialize;

/// The shoutbox's own look, against the `--hlin-*` tokens and the shared
