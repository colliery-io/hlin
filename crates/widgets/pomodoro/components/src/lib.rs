//! The focus timer's components: a ring that empties as the phase runs, the time
//! left in the middle, and the buttons that make sense for where it is.
//!
//! The widget says how long is left when asked; this counts down from
...
//! writes and their refusals) is `hlin-widget-ui`'s.
...
pub fn Pomodoro() -> impl IntoView {
    ...
    let drawn = loaded_view(timer, move |timer| {
