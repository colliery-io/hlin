//! The sparkline's components: one metric as a line over the surface's time
//! range, with where it is now, and its low and high.
//!
//! The range is the shell's: its time picker, sent in `context`, which the
//! client gives as `time_range`. Changing the picker is a change, which fetches again (`Widget::load` does that
//! for every widget); the request here reads the range to ask for it. On a
//! page, or on the widget's own UI, which have no picker, there is no range,
//! and the widget answers the last hour; this asks again each minute so that
//! hour moves on.
...
pub fn Sparkline() -> impl IntoView {
    ...
    set_interval(
        move || {
            if visible.get_untracked() && time_range.with_untracked(Option::is_none) {
    ...
    let drawn = loaded_view(drawn, move |drawn| {
